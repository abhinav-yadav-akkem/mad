...
public class MainActivity extends AppCompatActivity {
    ...
    public void show(View view) {
        ...
        if (c1.isChecked()) {
            ...
            if (value.equals("Left")) t1.setGravity(Gravity.LEFT);
            else if (value.equals("Right")) t1.setGravity(Gravity.RIGHT);
            else t1.setGravity(Gravity.CENTER);
        } else t1.setText("");
        if (c2.isChecked()) {
            t2.setText("ITALIC");
            t2.setTypeface(Typeface.defaultFromStyle(Typeface.ITALIC));
            if (value.equals("Left")) t2.setGravity(Gravity.LEFT);
            else if (value.equals("Right")) t2.setGravity(Gravity.RIGHT);
            else t2.setGravity(Gravity.CENTER);
        } else t2.setText("");
        if (c3.isChecked()) {
            t3.setText("NORMAL");
            if (value.equals("Left")) t3.setGravity(Gravity.LEFT);
            else if (value.equals("Right")) t3.setGravity(Gravity.RIGHT);
